function getTask(task_id){
  return formatTaskRows(getFilteredDataWithQuery({
    spreadsheetId:TaskSpreadSheetId,
    sheetName:TaskSheetName,
    filters:[
      {column:TASK_SPREAD_SHEET_COLUMNS.ID.column , operator:'=', value:task_id, connector:'AND'},
    ],
    skipRows:1,
    selectColumns:['*']
  }))
...
function getAssigningTask(user_id) {
  return formatTaskRows(getFilteredDataWithQuery({
    ...
    sheetName:TaskSheetName,
    filters:[
      {column:TASK_SPREAD_SHEET_COLUMNS.STATUS.column , operator:'=', value:TASK_STATUS.IN_PROGRESS.header_name, connector:'AND'},
      {column:TASK_SPREAD_SHEET_COLUMNS.ASSIGNEE.column , operator:'LIKE', value:user_id, connector:'AND'},
    ],
    skipRows:1,
    selectColumns:['*']
  }))
...
function getRequestTask(user_id) {
  return formatTaskRows(getFilteredDataWithQuery({
    ...
    filters:[
      {column:TASK_SPREAD_SHEET_COLUMNS.STATUS.column , operator:'=', value:TASK_STATUS.IN_PROGRESS.header_name, connector:'AND'},
      {column:TASK_SPREAD_SHEET_COLUMNS.REQUESTER.column , operator:'LIKE', value:user_id, connector:'AND'},
    ],
    skipRows:1,
    selectColumns:['*']
  }))
...
function postRemindTask() {
  const tasks = formatTaskRows(getFilteredDataWithQuery({
    spreadsheetId:TaskSpreadSheetId,
    sheetName:TaskSheetName,
    filters:[
      {column:TASK_SPREAD_SHEET_COLUMNS.ASSIGNEE.column , operator:'like', value:DEBUG_USER_ID, connector:'AND'},
      {column:TASK_SPREAD_SHEET_COLUMNS.STATUS.column , operator:'=', value:TASK_STATUS.IN_PROGRESS.header_name, connector:'AND'},
    ],
    skipRows:1,
    selectColumns:['*']
  ...
  Object.keys(groupedTasks).forEach(user => {
    postDirectMessage(
      user,
      makeTaskListBlocks(groupedTasks[user])
    );
  });
}
...
function formatTaskRows(rows){
  return rows.map(row => {
    return {
      ...
      assignee: row[TASK_SPREAD_SHEET_COLUMNS.ASSIGNEE.index].split(',').map(item => item.trim()),

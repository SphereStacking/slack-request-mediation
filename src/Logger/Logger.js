 ...
 *
 * @param {string|number|Array|Object} logData - ログとして記録するデータ。
 * @param {Object} [logLevel=LogLevel.INFO] - LogLevelの定数から指定されるログのレベルオブジェクト。
 * @param {number} [stackDepth=3] - 呼び出し元スタックの深さ。デフォルトは3。
 * @throws {Error} 指定されたシートが見つからない場合にエラーをスローします。
 */
function logToSheet(logData, logLevel = LogLevel.INFO, stackDepth = 3) {
  try {
 ...
 * @param {Object} logLevel - チェックするログレベルオブジェクト
 * @param {Object} currentLogLevel - 現在のログレベルオブジェクト
 * @returns {boolean} - 記録するべき場合はtrue、そうでない場合はfalse
 */
function shouldLog(logLevel, currentLogLevel) {
...
const SpreadsheetSingleton = (() => {
  ...
  function createInstance() {
    return SpreadsheetApp.openById(LoggerSpreadsheetAppID);
  }
